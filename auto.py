                ...
                if nowtime < starttime:
                    ...
                    for pro_id in pro_list:
                        ...
                        if pro['status'] == Service.Pro.STATUS_ONLINE:
                            continue
                        err, ret = yield from Service.Pro.update_pro(pro['pro_id'], pro['name'],
                                Service.Pro.STATUS_ONLINE, pro['class'], pro['expire'], None, None, pro['tags'])
                    ...
                    for pro_id in pro_list:
                        ...
                        if pro['status'] == Service.Pro.STATUS_HIDDEN:
                            continue
                        err, ret = yield from Service.Pro.update_pro(pro['pro_id'], pro['name'],
                                Service.Pro.STATUS_HIDDEN, pro['class'], pro['expire'], None, None, pro['tags'])
                        err, pro = yield from Service.Pro.get_pro(pro_id, None, True)
                    auto_list.remove(contn)
