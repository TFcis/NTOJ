...
class TestCommunicationConfig(unittest.TestCase):
    def test_custom_summary_directory_is_allowed(self):
        spec = CommunicationProblemSpec()
        config = spec.get_default_config()
        self.assertNotIn('res/summary', spec.get_allowed_file_paths(config, 1))
        config.summary_type = SummaryType.CUSTOM
        self.assertIn('res/summary', spec.get_allowed_file_paths(config, 1))
...
class TestCommunicationUnpack(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_unpack_builds_input_only_config(self, pack_service, pro_service):
        ...
        async def unpack(*args):
            backup = next(name for name in os.listdir('problem') if name.startswith('.backup-'))
            shutil.copytree(f'problem/{backup}/original', 'problem/1')
            return None, None
        pack_service.inst.unpack = AsyncMock(side_effect=unpack)
