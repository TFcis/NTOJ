...
class TestContestSession(unittest.TestCase):
    def test_hard_end_before_personal_start_is_ended_with_zero_duration(self):
        start = make_contest().contest_start
        session = ContestSession(
            contest_id=1, acct_id=2, start_time=start,
            end_time=start - datetime.timedelta(minutes=1),
        )
        self.assertEqual(session.duration, datetime.timedelta())
        self.assertIs(session.phase(session.end_time), ContestPhase.ENDED)
        self.assertIs(session.phase(start), ContestPhase.ENDED)
...
class TestFlexibleContestStart(unittest.IsolatedAsyncioTestCase):
    async def test_start_returns_persisted_session_and_invalidates_caches(self):
        ...
        self.assertIn("statement_timestamp()", query)
        self.assertIn("FOR UPDATE", db.fetchval.await_args.args[0])
