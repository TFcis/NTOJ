...
class FlexibleContestTimeTest(AsyncTest):
    async def main(self):
        ...
        await self._test_edit_active_session_limits()

    async def _test_edit_active_session_limits(self):
        now = datetime.datetime.now(datetime.UTC)
        start = now - datetime.timedelta(minutes=30)
        config = self._config(
            'flexible edited limits', (now - datetime.timedelta(hours=2)).replace(tzinfo=None),
            (now + datetime.timedelta(hours=2)).replace(tzinfo=None), 3600,
        )
        with AccountContext('admin@test', 'testtest') as admin:
            contest_id = await self._create_contest(
                admin, config, acct_ids=(5, 6), pro_ids=(8,),
            )
            res = admin.post(f'contests/{contest_id}/manage/pro', data={
                'reqtype': 'update_score_type', 'pro_id': 8,
                'score_type': ProblemScoreType.IOI2013,
            })
            self.assertAPIReturnSuccess(res.text)
            with AccountContext('contest2@test', 'test') as user:
                await self._start(user, contest_id)
                await self._set_sessions(contest_id, (
                    (5, start, start + datetime.timedelta(hours=1)),
                ))
                # Other session families must retain their own time windows.
                await ContestService.inst.db.execute(
                    'INSERT INTO contest_sessions '
                    '(contest_id, acct_id, session_type, start_time, end_time) '
                    'VALUES ($1, 5, 1, $2, $3)',
                    contest_id, start, now,
                )
                await self._insert_challenge(
                    contest_id, 8, 5, now - datetime.timedelta(minutes=2),
                    ChalConst.STATE_AC, 100,
                )
                self.assertEqual(self._scoreboard(admin, contest_id)[5]['total_score'], 100)
                # Warm both contest and scoreboard caches before each edit.
                for duration, hard_end, running in (
                    (600, now + datetime.timedelta(hours=2), False),
                    (3600, now + datetime.timedelta(hours=2), True),
                    (3600, now - datetime.timedelta(minutes=5), False),
                    (3600, start - datetime.timedelta(minutes=1), False),
                    (7200, now + datetime.timedelta(hours=2), True),
                ):
                    user.get(f'contests/{contest_id}/info')
                    self._scoreboard(admin, contest_id)
                    config['contest_duration'] = duration
                    config['contest_end'] = self.get_isoformat(hard_end.replace(tzinfo=None))
                    res = admin.post(f'contests/{contest_id}/manage/general', data=config)
                    self.assertAPIReturnSuccess(res.text)
                    row = await ContestService.inst.db.fetchrow(
                        'SELECT start_time, end_time FROM contest_sessions '
                        'WHERE contest_id = $1 AND acct_id = 5 AND session_type = 0', contest_id,
                    )
                    self.assertEqual(row['start_time'], start)
                    self.assertEqual(row['end_time'], min(
                        start + datetime.timedelta(seconds=duration), hard_end,
                    ))
                    other_end = await ContestService.inst.db.fetchval(
                        'SELECT end_time FROM contest_sessions '
                        'WHERE contest_id = $1 AND session_type = 1', contest_id,
                    )
                    self.assertEqual(other_end, now)
                    for path in ('proset', 'pro/8', 'submit/8'):
                        res = user.get(f'contests/{contest_id}/{path}')
                        if running:
                            self.assertEqual(res.status_code, 200)
                            self.assertNotIn('"Eacces"', res.text)
                        else:
                            self.assertAPIReturnValue(res.text, ('Eacces', 'Permission denied'))
                    self.assertEqual(
                        self._scoreboard(admin, contest_id)[5]['total_score'],
                        100 if running else 0,
                    )
                # Moving the global start must not move an existing personal start.
                config['contest_start'] = self.get_isoformat(
                    (now + datetime.timedelta(minutes=5)).replace(tzinfo=None)
                )
                res = admin.post(f'contests/{contest_id}/manage/general', data=config)
                self.assertAPIReturnSuccess(res.text)
                self.assertNotIn('"Eacces"', user.get(f'contests/{contest_id}/proset').text)
                config['contest_time_mode'] = ContestTimeMode.FIXED
                res = admin.post(f'contests/{contest_id}/manage/general', data=config)
                self.assertAPIReturnValue(res.text, (
                    'Etime', 'Contest time mode cannot be changed after the contest starts',
                ))
            with AccountContext('contest3@test', 'test') as pending:
                res = pending.post(f'contests/{contest_id}/info', data={'reqtype': 'start'})
                self.assertAPIReturnValue(res.text, (
