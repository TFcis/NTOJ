...
class ProService:
    ...
    async def get_pro(self, pro_id, acct=None, special=None):
        ...
        async with self.db.acquire() as con:
            ...
            name, status, clas, expire, tags = result['name'], result['status'], result['class'][0], result['expire'], \
                result['tags']
    ...
    async def list_pro(self, acct=None, state=False, clas=None, reload=False):
        ...
        if (prolist := (await self.rs.hget('prolist', field))) is not None and reload is False:
    ...
    async def _unpack_pro(self, pro_id, pack_type, pack_token):
        ...
        if pack_type == ProService.PACKTYPE_CONTHTML:
            ...
            try:
                ...
                os.symlink(os.path.abspath(f"problem/{pro_id}/http"),
...
class ProClassService:
    ...
    async def get_pubclass(self, pubclass_id):
        async with self.db.acquire() as con:
            res = await con.fetch('SELECT "pubclass_id", "name", "list" FROM "pubclass" WHERE "pubclass_id" = $1;',
                                  int(pubclass_id))
    ...
    async def update_pubclass(self, pubclass_id, pubclass_name, p_list):
        ...
        async with self.db.acquire() as con:
            await con.execute('UPDATE "pubclass" SET "name" = $1, "list" = $2 WHERE "pubclass_id" = $3', pubclass_name, p_list, pubclass_id)
