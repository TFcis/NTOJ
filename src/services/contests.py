...
class ContestService:
    ...
    async def start_official_session(self, contest: Contest, acct: Account):
        ...
        async with self.db.acquire() as con, con.transaction():
            # Serialize starts with timing edits; read settings after the lock.
            await con.fetchval(
                "SELECT contest_id FROM contest WHERE contest_id = $1 FOR UPDATE",
                contest.contest_id,
            )
    ...
    async def update_contest(self, acct: Account, contest: Contest, prolist_updated=False, userlist_updated=False):
        ...
        async with self.db.acquire() as con:
            async with con.transaction():
                ...
                if contest.contest_time_mode is ContestTimeMode.FLEXIBLE:
                    # The contest UPDATE holds the same row lock as Start.
                    await con.execute(
                        """
                        UPDATE contest_sessions
                        SET end_time = LEAST(
                            start_time + $2 * INTERVAL '1 second', $3
                        )
                        WHERE contest_id = $1 AND session_type = $4
                        """,
                        contest.contest_id,
        ...
        if contest.contest_time_mode is ContestTimeMode.FLEXIBLE:
            # Reload stored session ends instead of caching the request's old data.
            await self.rs.hdel('contest', str(contest.contest_id))
            await self.invalidate_scoreboard_cache(contest.contest_id)
        else:
            b_contest = pickle.dumps(contest)
