...
@asynccontextmanager
async def program_config_transaction(con, pro_id, problem_type, config):
    """Keep source renames reversible until the configuration transaction commits."""
    from services.chal import COMPILER_INFOS
    from services.pro import ProType
    from services.prospec.communication import normalize_submission_format
        ...
        async with con.transaction():
            ...
            if problem_type == ProType.COMMUNICATION:
                row = await con.fetchrow(
                    'SELECT problem_type, config FROM problem WHERE pro_id = $1 FOR UPDATE',
                    pro_id,
                )
                if row and row['problem_type'] == int(ProType.COMMUNICATION):
                    old_config = json.loads(row['config'])
                    old_format = normalize_submission_format(old_config.get('submission_format', ('main.%l',)))
                    new_format = normalize_submission_format(config.spec_config.submission_format)
                    if old_format != new_format:
                        challenges = await con.fetch(
                            'SELECT chal_id, compiler_type FROM challenge WHERE pro_id = $1',
                            pro_id,
                        )
                        for challenge in challenges:
                            ext = COMPILER_INFOS[challenge['compiler_type']].source_ext
                            pairs = rename_submission_files(
                                challenge['chal_id'],
                                [name.replace('%l', ext) for name in old_format],
                                [name.replace('%l', ext) for name in new_format],
                                dry_run=True,
                            )
                            if pairs:
                                plans.append((challenge['chal_id'], pairs))
            for chal_id, pairs in plans:
                rename_submission_files(chal_id, [a for a, _ in pairs], [b for _, b in pairs])
                completed.append((chal_id, pairs))
            yield
    except BaseException:
        for chal_id, pairs in reversed(completed):
            try:
                rename_submission_files(chal_id, [b for _, b in pairs], [a for a, _ in pairs])
            except OSError:
                logger.exception('Failed to restore source filenames for challenge %s', chal_id)
        raise
