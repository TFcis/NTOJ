...
async def unpack_program_package(
    ...
    try:
        if os.path.exists(problem_dir):
            backup_dir = tempfile.mkdtemp(prefix=f".backup-{pro_id}-", dir="problem")
            os.rename(problem_dir, f"{backup_dir}/original")
        unpack_started = True
        ...
        err, _ = await ProService.inst.update_pro_config(pro_id, problem_type, config)
        if err:
            return err, None
        failed = False
        ...
        if failed and unpack_started and os.path.exists(problem_dir):
            shutil.rmtree(problem_dir)
        if backup_dir is not None:
            if failed and os.path.exists(f"{backup_dir}/original"):
                os.rename(f"{backup_dir}/original", problem_dir)
