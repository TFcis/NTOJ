...
class ManageProClassHandler(RequestHandler):
        ...
        if page is None:
            ...
            proclass_list = list(
                filter(
                    ...
                    in (ProClassConst.OFFICIAL_PUBLIC, ProClassConst.OFFICIAL_HIDDEN),
                    proclass_list,
                )
            )
            ...
            await self.render(
                ...
                pageoff=pageoff,
                proclass_total_cnt=proclass_total_cnt,
            )
    ...
    async def add_proclass(self):
        ...
        err, proclass_id = await ProClassService.inst.add_proclass(
            name, p_list, desc, None, proclass_type
        )
        if err:
            return self.error(err)
