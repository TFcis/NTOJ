...
with tempfile.TemporaryDirectory() as working_path:
    ...
    web_dir = config.WEB_PROBLEM_STATIC_FILE_DIRECTORY[
        0:-8
    ]  # /srv/oj_web/oj/problem, remove `/problem`
    ...
    shutil.copy(
        f"{working_path}/NTOJ/src/static/challist.css", f"{web_dir}/challist.css"
    )
    shutil.copy(
        f"{working_path}/NTOJ/src/static/manage-pro.css", f"{web_dir}/manage-pro.css"
    )
    ...
    shutil.copy(f"{working_path}/NTOJ/src/static/submit.css", f"{web_dir}/submit.css")
    shutil.copytree(
        f"{working_path}/NTOJ/src/static/third", f"{web_dir}/third", dirs_exist_ok=True
    )
    ...
    subprocess.run(
        [
            f"$HOME/.local/bin/poetry -P {os.getcwd()} run python3 {working_path}/NTOJ/migration/migration.py"
        ],
