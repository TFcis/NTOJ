...
class ContestService:
    ...
    def set(self, cont_name, clas, status, start, end, pro_list=None, acct_list=None):
        ...
        if cont_name == 'default':
            ...
            self.rs.set(cont_name + '_contest', msgpack.packb({
                'status'    : status,
                'start'     : start,
                'end'       : end,
                'pro_list'  : pro_list,
...
class BoardHandler(RequestHandler):
    ...
    def get(self):
        ...
        if cont_name == 'default':
            ...
            err, prolist = yield from Service.Pro.list_pro(acct=self.acct, clas=clas)
            err, acctlist = yield from Service.Rate.list_rate(acct=self.acct, clas=clas)
